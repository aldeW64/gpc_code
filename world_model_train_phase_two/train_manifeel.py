"""
Phase-2 world model training on ManiFEEL data.
...
Differences from train.py (PushT):
  ...
  - Loads a Phase-1 ManiFEEL checkpoint from config['phase_one_checkpoint']
  - Everything else — sigma schedule, UNet shape, optimiser, save cadence —
    is identical to the PushT phase-2 training script.

Run from repo root:
    python -m world_model_train_phase_two.train_manifeel \
        --config world_model_train_phase_two/configs/train_manifeel_phase_two_config.yml
Or from within world_model_train_phase_two/:
...
try:
    # Supports running from within world_model_train_phase_two/.
    ...
    # Supports running from repo root with python -m world_model_train_phase_two.train_manifeel.
    from .dataset_manifeel import build_manifeel_dataset
    from .utils import save
    from .diffusion.denoiser_manifeel import (
        Denoiser,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Phase-2 world model training on ManiFEEL dataset.')
    parser.add_argument(
        '--config',
        type=str,
    ...
    if config.get('wandb', False):
        wandb.init(
            project='world_model_train_phase_two_manifeel',
            config=config,
            name='world_model_train_phase_two_manifeel',
        )
    else:
    ...
    with tqdm(range(1, num_epochs + 1), desc='Epoch') as tglobal:
        for epoch_idx in tglobal:
            ...
            # Save every epoch (same cadence as original phase-2 train.py)
            if epoch_idx % 1 == 0:
