"""
Denoiser variant for ManiFEEL training.

Identical to denoiser.py except it imports InnerModel and InnerModelConfig
from inner_model_manifeel (which supports configurable action_dim, default 7)
instead of from inner_model (which hardcodes action_dim=2 for PushT).

All training logic, sigma schedules, and loss computations are unchanged.
"""
...
from .inner_model_manifeel import InnerModel, InnerModelConfig  # noqa: F401 (re-exported)
...
class Denoiser(nn.Module):
    ...
    def forward(self, batch: Batch, device):
        ...
        b, t, c, h, w = obs.size()
        H, W = (
            (self.cfg.upsampling_factor * h, self.cfg.upsampling_factor * w)
        ...
        for i in range(seq_length):
            ...
            prev_act = None if self.is_upsampler else act[:, i: n + i]
            obs = all_obs[:, n + i]
