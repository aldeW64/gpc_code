"""
Diffusion sampler (unchanged from the original phase-two implementation).

Implements the Karras et al. (2022) deterministic / stochastic ODE sampler
with optional Heun's method (order=2) and stochastic noise injection (s_churn).
"""
...
class DiffusionSampler:
    def __init__(self, denoiser: Denoiser, cfg: DiffusionSamplerConfig) -> None:
        ...
        self.sigmas = build_sigmas(
            cfg.num_steps_denoising, cfg.sigma_min, cfg.sigma_max, cfg.rho, denoiser.device
        )

    @torch.no_grad()
    def sample(
        self,
        prev_obs: Tensor,            # (B, T, C, H, W)  past fused frames
        prev_act: Optional[Tensor],  # (B, T, 7)  past actions
    ) -> Tuple[Tensor, List[Tensor]]:
        device = prev_obs.device
        b, t, c, h, w = prev_obs.size()
        # Flatten the time and channel dimensions for the denoiser
        prev_obs = prev_obs.reshape(b, t * c, h, w)

        s_in = torch.ones(b, device=device)
        gamma_ = min(self.cfg.s_churn / (len(self.sigmas) - 1), 2**0.5 - 1)

        # Start from pure noise with the same shape as a single fused frame
        ...
        for sigma, next_sigma in zip(self.sigmas[:-1], self.sigmas[1:]):
            ...
            if self.cfg.s_cond > 0:
                ...
                prev_obs_noised = self.denoiser.apply_noise(prev_obs, sigma_cond, sigma_offset_noise=0)
            else:
                sigma_cond = None
                prev_obs_noised = prev_obs

            denoised = self.denoiser.denoise(x, sigma * s_in, sigma_cond, prev_obs_noised, prev_act)
            ...
            if self.cfg.order == 1 or next_sigma == 0:
                ...
                denoised_2 = self.denoiser.denoise(
                    x_2, next_sigma * s_in, sigma_cond, prev_obs_noised, prev_act
                )
