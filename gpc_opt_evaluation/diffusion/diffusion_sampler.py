...
class DiffusionSampler:
    ...
    def sample(self, prev_obs: Tensor, prev_act: Optional[Tensor], random_start=True, save_start=None) -> Tuple[Tensor, List[Tensor]]:
        ...
        if random_start:
            ...
            save_start = copy.deepcopy(x)
        else:
            x = copy.deepcopy(save_start)
        trajectory = [x]
        ...
        for sigma, next_sigma in zip(self.sigmas[:-1], self.sigmas[1:]):
            ...
            trajectory.append(x)
        if random_start:
            return x, trajectory, save_start
        else:
            return x, trajectory
