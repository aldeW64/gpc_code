"""
InnerModel variant for ManiFEEL (7-DOF actions).

Identical to inner_model.py except:
  - InnerModelConfig gains an ``action_dim`` field (default 7).
  - act_emb uses ``nn.Linear(action_dim, ...)`` instead of ``nn.Linear(2, ...)``.

The denoiser and all other training infrastructure are shared unchanged.
"""
...
class InnerModelConfig:
    ...
    action_dim: int = 7                    # 7 for ManiFEEL; 2 for PushT
    num_actions: Optional[int] = None      # kept for API compatibility
...
class InnerModel(nn.Module):
    def __init__(self, cfg: InnerModelConfig) -> None:
        ...
        self.act_emb = nn.Sequential(
            nn.Linear(action_dim, cfg.cond_channels // cfg.num_steps_conditioning),
            nn.ReLU(),
