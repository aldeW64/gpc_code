...
class SpaceEfficientWandbLogger(WandbLogger):
    """Space efficient Wandb logger.

    A wandb logger that by default overrides artifacts to save space, instead of
    creating new version. A variable expiration_days can be set to control how long
    older versions of artifacts are kept. By default, the latest version is kept
    indefinitely, while older versions are kept for 5 days.
    """
    ...
    def _scan_and_log_checkpoints(self, checkpoint_callback: ModelCheckpoint) -> None:
        ...
        #         },
        #     }
        #     if not self._checkpoint_name:  # type: ignore
        ...
        #         artifact.wait()
        #     artifact.ttl = timedelta(days=self.expiration_days)  # type: ignore
        #     artifact.save()
        # self._last_artifacts = artifacts


class OfflineWandbLogger(SpaceEfficientWandbLogger):
    """Offline Wandb logger.
    ...
    def log_metrics(
        ...
        """Log metrics to wandb and trigger sync occasionally."""
