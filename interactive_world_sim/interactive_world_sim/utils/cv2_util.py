...
def draw_reticle(
    img: np.ndarray, u: float, v: float, label_color: tuple = (0, 255, 0)
) -> None:
    """Draws a reticle (cross-hair) on the image
...
def draw_text(
    ...
    *,
    ...
    uv_top_left_np = np.array(uv_top_left, dtype=float)
    ...
    for line in text.splitlines():
        ...
        uv_bottom_left_i = uv_top_left_np + np.ndarray([0, h])
        ...
        uv_top_left_np += np.ndarray([0, h * line_spacing])
...
def optimal_row_cols(
    n_cameras: int, in_wh_ratio: float, max_resolution: tuple = (1920, 1080)
