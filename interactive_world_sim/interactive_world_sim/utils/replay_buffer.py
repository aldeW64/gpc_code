...
def get_optimal_chunks(shape, dtype, target_chunk_bytes=2e6, max_chunk_length=None):
    """Get optimal chunks for a given shape and dtype.
...
class ReplayBuffer:
    """ReplayBuffer for storing data more efficiently.
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for _, value in root["data"].items():
            assert value.shape[0] == root["meta"]["episode_ends"][-1]
        self.root: zarr.Group = root

    # ============= create constructors ===============
    @classmethod
    def create_empty_zarr(cls, storage=None, root=None):
        """Create an empty zarr buffer."""
        if root is None:
            if storage is None:
        ...
        root["meta"]["episode_ends"] = np.zeros((0,), dtype=np.int64)
        return cls(root=root)

    @classmethod
    def create_empty_numpy(cls):
        """Create an empty numpy buffer."""
        root = {
            "data": dict(),
    ...
    def create_from_path(cls, zarr_path, mode="r", **kwargs):
        """Create a in-memory buffer from a on-disk zarr
    ...
    def copy_from_store(
        ...
        store=None,
        keys=None,
        chunks: Dict[str, tuple] = dict(),  # noqa
        compressors: Union[dict, str, numcodecs.abc.Codec] = dict(),  # noqa
        if_exists="replace",
        **kwargs,
    ):
        """Load to memory."""
        src_root = zarr.group(src_store)
        root: Union[zarr.Group, None] = None
        if store is None:
            # numpy backend
            meta = dict()
    ...
    def copy_from_path(
        ...
        **kwargs,
        ...
        """Copy from on-disk zarr to in-memory zarr.
    ...
    def save_to_store(
        self,
        store,
        chunks: Optional[Dict[str, tuple]] = dict(),  # noqa
        compressors: Union[str, numcodecs.abc.Codec, dict] = dict(),  # noqa
        if_exists="replace",
        **kwargs,
    ):
        ...
        for key, value in self.root["data"].items():
            cks = self._resolve_array_chunks(
                chunks=chunks, key=key, array=value  # type: ignore
            )
